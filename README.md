...
우리가 x.f(args)를 호출한다고 말하고, 암시적 파라미터 x는 C클래스의 객체로 선언된다.
결과는 이렇다:

1. 컴파일러는 객체의 선언된 타입과 메소드 이름을 본다.
...
이제 컴파일러는 호출되는 메소드의 가능한 모든 후보자들을 안다.

2. 다음, 컴파일러는 메소드 호출에서 공급되는 인수의 타입을 결정한다.
만약 f로 호출되는 모든 메소드들 사이에 공급되는 인수에 매개 변수 타입이 가장 매치되는 독특한 메소드가 있다면, 그 메소드는 호출되기 위해 선정된다.
이 과정은 오버로딩 검색?(overloading resolution) 이라고 불린다.
예를 들면, x.f("Hello") 호출에서, 컴파일러는 f(int)가 아닌 f(String)을 선택한다.
이 상황은 타입 변환(int 에서 double, Manager 에서 Employee, 기타 등등)때문에 컴플렉스를 가질 수 있다.
만약 그 컴파일러가 매개변수와 매치되는 어떤 메소드를 찾을 수 없거나 다중의 메소드들이 변환이 적응된 후에 모두 매치된다면, 그 컴파일러는 오류를 제출한다.
이제 그 컴파일러는 호출되는 것을 필요로 하는 메소드의 이름과 매개변수 타입을 안다.

    ```
    필기 : 메소드의 이름과 매개변수 타입 목록은 메소드의 시그니처로 호출된다는 것을 상기해라.
    예를 들면, f(int)와 f(String)은 같은 이름이지만 다른 시그니처를 가진 두 메소드이다.
    만약 당신이 슈퍼클래스 메소드와 같은 시그니처를 가지고 있는 서브클래스에서 메소드를 정의한다면, 당신은 슈퍼클래스 메소드를 재정의한다.

    그 반환 타입은 시그니처의 일부가 아니다.
    그러나, 당신이 메소드를 재정의 할 때, 당신은 호환 가능한 반환 타입을 유지하는 것을 필요로 한다.
    ...
    manager은 buddy와 같은 낮은 employee를 가지는 것을 결코 원하지 않을 것이다.
    Manager 서브클래스는 
    ...
    ```
3. 만약 메소드가 private, static, final, 또는 생성자라면, 컴파일러는 정확하게 어떤 메소드를 호출할 지 안다. (final 수식어는 다음 섹션에서 설명된다.)
이것은 정적 바인딩(static binding)이라 불린다.
그렇지 않으면, 그 호출되는 메소드가 암시적 매개변수의 실제 타입에 의존하고, 동적 바인딩은 실행 중에 사용되어야 한다.
예를 들면, 그 컴파일러는 동적바인딩과 함께 f(String)를 호출하는 명령어를 생성할 것이다.

4. 프로그램이 실행하고 메소드를 호출하기 위해 동적바인딩을 사용할 때, 그 가상머신은 x가 참조한 객체의 실제 타입에 적합한 메소드의 버전을 호출하여야 한다.
실제 타입이 C의 서브클래스, D라고 말하자.
만약 D클래스가 f(String)메소드를 정의한다면, 그 메소드는 호출된다.
만약 아니라면, D의 슈퍼클래스는 f(String)메소드와 기타 등을 탐색한다.
...
만약 그 호출이 super.f(param)이라면, 컴파일러는 그 암시적 매개변수의 슈퍼클래스의 메소드 테이블을 찾아본다.
...
그 getSalary메소드는 private, static, final이 아니라서, 동적으로 결합한다.
그 가상머신은 Employee와 Manager클래스의 메소드 테이블을 소개한다.
Employee 테이블은 모든 메소드가 Employee클래스에서 정의된 것이라고 보여준다:
...
실제로, 그것은 전체의 이야기가 아니다 - 당신이 나중에 이 장을 본다면, Employee클래스는 많은 메소드를 상속하는 슈퍼클래스 Object를 가지고 있다.
우리는 이제 Object메소드를 무시한다.

그 Manager메소드 테이블은 미세하게 다르다.
세 개의 메소드는 상속 받고, 한 메소드는 재정의 되고, 한 메소드는 추가된다.
...
저것은 Employee, Manager, 또는 Employee의 또 다른 서브클래스의 테이블이 될지도 모른다.
2. 그러면, 그 가상머신은 getSalary()시그니처를 정의한 클래스를 탐색한다.
이제 어떤 메소드를 호출할 지 안다.
3. 마지막, 가상머신은 그 메소드를 호출한다.

동적결합(동적바인딩)은 매우 중요한 속성을 가진다:
...
```
!주의 : 당신이 메소드를 재정의 할 때, 그 서브클래스 메소드는 적어도 슈퍼메소드만큼 보여야 한다.
특히, 만약 그 슈퍼클래스 메소드가 public 이라면, 서브클래스 메소드는 오직 public 으로 선언되어야 한다.
우연히 서브클래스 메소드의 public 지정자를 생략하는 것은 흔한 오류이다.
그러면 컴파일러는 당신이 더 제한적인 접근 권한을 제공하려고 하는 것을 불평한다.
```
...
간단히 final 수식어를 사용하여 클래스를 선언해라, 다음과 같이:
...
만약 당신이 이것을 한다면, 어떤 서브클래스도 그 메소드를 재정의 할 수 없다. (final클래스에서 모든 메소드는 자동으로 final이다.)
예를 들면:
...
```
필기 : 필드는 오직 final로 선언될 수 있다는 것을 상기해라.
final필드는 그 객체가 생성된 후에 변경될 수 없다.
그러나, 만약 클래스가 final로 선언된다면, 필드가 아닌, 메소드들은 자동으로 final이다.
```

오직 final메소드 또는 클래스를 만드는 한가지 좋은 이유가 있다:
서브클래스에서 그것의 의미가 변경될 수 없도록 하기 위해서이다.
예를 들면, Calendar클래스의 getTime과 setTime메소드는 final이다.
이것은 Calendar클래스의 설계자가 Date클래스와 calendar state 사이의 변환을 위한 책임을 떠맡았다는 것을 나타낸다.
어떤 서브클래스도 이 방식을 엉망으로 만드는 것이 허락되서는 안된다.
...
저것은 조금 극단적인 것일지도 모르지만, 당신이 클래스 계급을 설계할 때, 우리는 final메소드와 클래스에 대해 신중히 생각하는 것은 좋은 생각이라는 것에 동의한다.

자바 초기에, 몇몇 프로그래머들은 동적바인딩의 부담?(overhead)을 피하기 위해 final키워드를 사용했다.
만약 메소드가 재정의되지 않고, 짧다면, 컴파일러는 그 메소드 호출을 최적화 할 수 있다 - 과정은 *inlining* 이라 불린다.
예를 들면, e.getName()호출을 인라이닝하는 것은 e.name에 접근하는 필드로 대체한다.
이것은 개선할만한 가치가 있다 -  CPU는 현재 명령을 실행하는 동안 그들의 명령어를 예비추출?(prefetching) 하는 전략을 방해하기 때문에 분기를 싫어한다.
그러나, 만약 getName이 또 다른 클래스에서 재정의 될 수 있다면, 컴파일러는 재정의 코드가 무엇을 할지 아는 방법이 없기 때문에 인라인할 수 없다.

운좋게도, 가상머신에서 적시 컴파일러는 전통적인 컴파일러보다 더 나은 작업을 수행할 수 있다.
어떤 클래스가 주어진 클래스를 상속하는지 확실하게 알 수 있고, 어떤 클래스가 주어진 메소드를 실제로 재정의할 수 있는지 확인할 수 있다.
...
3장에서 한 타입에서 또 다른 타입으로의 변환을 강제하는 과정을 캐스팅이라 불리는 것을 상기해라.
...
당신이 캐스트를 만들기를 원하는 한 가지 이유가 있다 - 그것의 실제 타입이 일시적으로 잊어버린 후에 최대 가용성에서 객체를 사용하는 것
예를 들면, ManagerTest클래스에서, 그것의 몇몇 요소들이 보통 employees였기 때문에 staff배열은 Employee객체의 배열이 되어야 한다.
...
우리는 manager의 bonus를 설정하는 옳은 타입을 필요로 했다.)

당신도 알다시피, 자바에서 모든 변수는 타입을 가지고 있다.
타입은 변수가 참조하고 그것이 할 수 있는 객체의 종류를 묘사한다. 그것이 할 수 있는 
...
프로그램이 실행할 때, 자바 런타임 시스템은 깨진 약속을 알리고, ClassCastException 이 발생한다.
...
마침내, 컴파일러는 만약 성공적인 캐스트의 기회가 없다면 당신이 캐스트 하는 것을 시키지 않을 것이다.
예를 들면, 캐스트

`String c = (String) staff[1];`

는 compile-time 오류이다. 왜냐하면 String은 Employee의 서브클래스가 아니기 때문이다.

요약해서 말하면:
- 당신은 상속 계층 내에 캐스트 할 수 있다
- 슈퍼클래스에서 서브클래스로 캐스팅 전에 확인하기 위해 instanceof를 사용해라.

```
필기 : 그 테스트
x instanceof C
는 만약 x가 null이라면 예외를 발생하지 않는다.
단순히 false를 반환한다.
그것은 이해하기 쉽다:
...
당신이 상속 계층을 상승함으로써, 클래스는 더 일반적이고 아마 더 추상적이게 된다.
...
당신은 항상 공통 필드와 메소드(추상이거나 아니거나)를 슈퍼클래스(추상이거나 아니거나)로 움직여야 한다.
```
...
당신도 알다시피, 클래스 안에 필드는 private로 가장 잘 태그되고, 메소드들은 보통 public으로 태그된다.
private로 선언되는 어떤 특징들은 다른 클래스에서 보이지 않을 것이다.
우리가 이 장의 시작 부분에서 말했듯이, 이것은 또한 서브클래스를 위한 진실이다:
서브클래스는 슈퍼클래스의 private필드에 접근할 수 없다.

그러나, 당신이 오직 서브클래스에서 메소드를 제한하기를 원하는 순간이 있다. 또는, 덜 흔하게, 서브클래스 메소드가 슈퍼클래스 필드에 접근하는 것을 허락하기 위해.
그 경우에는, 당신은 protected로 클래스를 선언한다.
예를 들면, 만약 Employee 슈퍼클래스가 private 대신에 protected로서 HireDay필드를 선언한다면, Manager메소드는 직접 접근할 수 있다.

그러나, Manager클래스 메소드들은 다른 Employee객체가 아닌, Manager객체의 HireDay필드에서 peek 할 수 있다.
이 제한은 단지 protected필드에 접근하기 위해 서브클래스를 형성함으로써 protected메커니즘을 남용할 수 없도록 만들어진다.

실제는, 신중하게 protected필드를 사용해라.
...
이것은 서브클래스(아마 그들의 조상을 잘 아는)가 메소드를 올바르게 사용하는 것이 신뢰될 수 있다는 것을 나타내지만, 다른 클래스는 할 수 없다.

메소드의 이런 종류의 좋은 예시는 Object클래스의 clone메소드이다 - 6장에서 더 자세하게 보자.

```
C++ 필기 : 그것이 일어남으로써, 자바에서 protected특징은 같은 패키지에서 모든 다른 클래스들 뿐만 아니라 모든 서브클래스에서 보일 수 있다.
이것은 C++에서 protected의 의미가 미세하게 다르고, C++보다 Java에서 덜 안전하게 protected의 개념을 만든다.
```
